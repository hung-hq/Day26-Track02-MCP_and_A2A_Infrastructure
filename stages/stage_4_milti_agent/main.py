...
def search_privacy_law(query: str) -> str:
    """Search privacy and data protection law knowledge base for applicable frameworks.

    Args:
        query: Natural language query about privacy, GDPR, CCPA, data protection.
    """
    knowledge = [
        (
            ["gdpr", "european", "eu", "data"],
            "GDPR: up to EUR 20M or 4% of global revenue. Data breach notification within 72 hours. "
            "Data Subject Rights (DSAR): 30-day response. DPA appointment required. "
            "Lawful basis required for all processing (Article 6). Privacy impact assessments mandatory.",
        ),
        (
            ["ccpa", "california", "state", "consumer"],
            "CCPA: up to $7,500 per intentional violation. CPRA (California Privacy Rights Act): "
            "expanded rights + $2,500 per violation ($7,500 intentional). Consumer rights: access, delete, "
            "opt-out, portability. Class action exposure ($100-$750 per consumer per incident).",
        ),
        (
            ["breach", "notification", "incident", "hipa"],
            "HIPAA: fines up to $1.5M per category per year. Breach notification: 60 days. "
            "State notification laws: 30+ states require notification. Business Associate Agreement required. "
            "Medical records protection under 45 CFR §§ 164.100-414.",
        ),
    ]
    query_lower = query.lower()
    results = []
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves GDPR, CCPA, data protection, privacy, data breach'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    # Privacy specialist system prompt
    privacy_prompt = (
        "You are a specialized privacy counsel expert in GDPR, CCPA, HIPAA, and international "
        "data protection regulations. You advise on data breach risks, privacy compliance frameworks, "
        "Data Subject Rights, DPA requirements, and privacy impact assessments. "
